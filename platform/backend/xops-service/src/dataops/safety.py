...
카탈로그는 신뢰 대상이 아니다: `POST /dataops/catalog` 로 `data:write` 토큰 보유자가 소스를
등록할 수 있고(dev는 토큰 발급이 개방) object·컬럼명·range 가 생성 SQL에 그대로 조립되므로
UNION·불리언 주입 경로가 된다. 인증은 등록 주체를 좁힐 뿐 이 경로 자체를 없애지 못한다.
그래서 3중으로 막는다.
